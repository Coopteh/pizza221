...
class OrderController
{
    ...
    private function sendOrderConfirmation(array $orderData, $orderId): bool
    {
        ...
        try {
            ...
            // Email content
            $mail->setFrom('v.milevskiy@coopteh.ru', 'Strax');
            $mail->addAddress($orderData['email']);
            $mail->addReplyTo('no-reply@coopteh.ru', 'No Reply');
            $mail->isHTML(true);
            $mail->Subject = 'Ваш заказ #' . $orderId . ' в Strax';
...
<body>
    ...
    <p>С уважением,<br>Команда Страхование ККТ</p>
</body>
</html>
HTML;
    }
    ...
    private function buildTextEmailBody(array $orderData, $orderId): string
    {
        ...
        return <<<TEXT
Спасибо за ваш заказ в Страхование ККТ!
