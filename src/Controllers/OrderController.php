...
class OrderController
{
    ...
    public function create(): string
    {
        ...
        $_SESSION['basket'] = [];
        $_SESSION['flash'] = "Спасибо! Ваш абонемент успешно оформлен и передан администриции.";
    ...
    private function sendOrderConfirmation(array $orderData, $orderId): bool
    {
        ...
        try {
            ...
            // Email content
            $mail->setFrom('v.milevskiy@coopteh.ru', 'Trenazherniy zal');
            $mail->addAddress($orderData['email']);
            $mail->addReplyTo('no-reply@coopteh.ru', 'No Reply');
            $mail->isHTML(true);
            $mail->Subject = 'Ваш заказ #' . $orderId . ' в Trenazherniy zal';
...
<body>
    <h2 class="header">Спасибо за ваш заказ в Тренажерный зал "Олимп"!</h2>
    ...
    <p>С уважением,<br>Команда Тренажерный зал "Олимп"</p>
</body>
</html>
HTML;
    }
    ...
    private function buildTextEmailBody(array $orderData, $orderId): string
    {
        ...
        return <<<TEXT
Спасибо за ваш заказ в Тренажерный зал "Олимп"!
...
Информация об оформлении заказа:
ФИО: {$orderData['fio']}
